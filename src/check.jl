...
function check_configuration!(user_conf::Vector{Any}, data_folder_path::String, verbosity_random_walk::Int64, verbosity_flux::Int64, chain_id::Int64)
  ...
  if (isfile("vertex_ampls/$(M)/vertex_j=$(j).jld2") == false)
    error("The vertex amplitude for $(M) model and j=$(j) does not exists in the current directory. Unless the latter was removed, try to run the code from inside the main folder")
  end

  # (j, M, N, b, σ)  check
  if (typeof(j) != Float64 || j < 0)
    error("Please assign spin as positive float in user_conf $(user_conf)\n")
  end
  check_model = (M == "BF" || M == "EPRL")
  if (check_model == false)
    error("I don't understand the model you want to compute in user_conf $(user_conf)\n")
  end
  if (typeof(N) != Int64 || N <= 0)
    error("I don't understand number of iterations in user_conf $(user_conf)\nPlease use a positive integer number")
  end
  if (b > N)
    error("burnin is greater than number of iterations in user_conf $(user_conf)")
  end
  if (typeof(σ) != Float64 || σ <= 0)
    error("Please assign standard deviation as positive float in user_conf $(user_conf)\n")
  end

  # random walk flags check
  if (typeof(add_chains) != Bool)
    error("add_chains must have a bool value in user_conf $(user_conf)")
  end
  if (add_chains == true && random_walk == false)
    error("You can't pretend to add chains if you don't do random walk in user_conf $(user_conf)")
  end

  # angles flags check
  if (typeof(compute_angles) != Bool)
    error("compute_angles must have a bool value in user_conf $(user_conf)")
  end
  if (typeof(compute_angles_correlations) != Bool)
    error("compute_angles_correlations must have a bool value in user_conf $(user_conf)")
  end

  # volumes flags check  
  if (typeof(volumes_correlations_node_1) != Int64 || typeof(volumes_correlations_node_2) != Int64)
    error("In user_conf $(user_conf) the nodes for the volumes correlations must be integers")
  end
  if (volumes_correlations_node_1 < 1 || volumes_correlations_node_1 > 20)
    error("In user_conf $(user_conf), node 1 for the volumes correlations must be between 1 and 20")
  end
  if (volumes_correlations_node_2 < 1 || volumes_correlations_node_2 > 20)
    error("In user_conf $(user_conf), node 2 for the volumes correlations must be between 1 and 20")
  end

  # entropy flags check   
  if (typeof(compute_density_matrix) != Bool)
    error("compute_density_matrix must have a bool value in user_conf $(user_conf)")
  end
  if (isempty(nodes_in_subsystem) && compute_density_matrix == true)
    error("In user_conf $(user_conf) you chose to compute the density matrix but you didn't specified the nodes in subsystem")
  end
  if (size(nodes_in_subsystem)[1] > 20)
    error("In user_conf $(user_conf) there are more than 20 nodes for the subsystem")
  end
  for node in nodes_in_subsystem
    if (typeof(node) != Int || node < 1 || node > 20)
      error("In user_conf $(user_conf) the node $(node) is not a valid option for the computation of the density matrix. Use an integer between 1 and 20")
    end
  end
...
function check_stored_operators!(user_conf::Vector{Any}, conf::Configuration, data_folder_path::String, verbosity_flux::Int64, chain_id::Int64, number_of_chains::Int64)
  ...
  if (number_of_existing_draws != number_of_existing_ampls)
    error("There are different numbers of stored draws and ampls of user_conf $(user_conf)")
  end
  user_conf[6] = convert(Array{Any}, user_conf[6])
  push!(user_conf[6], number_of_existing_draws) # this is user_conf[6][4]           

  if (verbosity_flux > 1 && chain_id == 1)
    println("Found $(number_of_existing_draws) draws previously stored for the $(M) config with j=$(j)")
  end
  #----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if (number_of_existing_angles != number_of_existing_angles_sq)
    error("There are different numbers of stored angles and angles sq of user_conf $(user_conf)")
  end
  ...
  if (verbosity_flux > 1 && chain_id == 1)
    println("Found $(number_of_existing_angles) angles previously stored for the $(M) config with j=$(j)")
  end
  if (verbosity_flux > 1 && chain_id == 1)
    println("Found $(number_of_existing_angles_pseudo_correlations) operators <A_n,A_m> previously stored for the $(M) config with j=$(j)")
  end
  #----------------------------------------------------------------------------------------------------------------------------------------------------------------------------                    
  ...
  if (number_of_existing_volumes != number_of_existing_volumes_sq)
    error("There are different numbers of stored volumes and volumes sq of user_conf $(user_conf)")
  end
  ...
  if (verbosity_flux > 1 && chain_id == 1)
    println("Found $(number_of_existing_volumes) volumes previously stored for the $(M) config with j=$(j)")
  end
  if (verbosity_flux > 1 && chain_id == 1)
    println("Found $(number_of_existing_volumes_pseudo_correlations) operators <V_$(volumes_correlations_node_1),V_$(volumes_correlations_node_2)> previously stored for the $(M) config with j=$(j)")
  end
  #----------------------------------------------------------------------------------------------------------------------------------------------------------------------------    
  ...
  if (verbosity_flux > 1 && chain_id == 1)
    println("Found $(number_of_existing_density_matrices) density matrices of subsystem $(conf.subsystem) previously stored for the $(M) config with j=$(j)")
  end
  ...
  if (conf.random_walk == false)
    if (number_of_existing_draws < number_of_chains)
      error("In user_conf $(user_conf) you said that you don't want to do random walk, but there are $(conf.total_draws_already_stored) draws already stored and you are using $(number_of_chains) chains")
    end
  end # if check on RW
